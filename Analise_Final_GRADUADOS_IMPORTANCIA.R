...
dados <- read.csv("dados_fatorial_IMP_final.csv", header = TRUE, sep = ";", encoding = "Windows-1258")
...
qgraph(cor(matcor), title="Correlações Importância - Graduados",
       # layout = "spring", 
...
qgraph(cor(matcor), layout="spring", shape="rectangle", 
       ...
       title = "Correlações Importância - Graduados")
...
gpca <- fviz_eig(pca, addlabels=FALSE, ylim = c(0,50), title = "")
...
gpca <- gpca +
  ...
  ggtitle("Scree Plot - Importância - Graduados") +
  ylab("Porcentagem de Variância Explicada") +
  xlab("Dimensões")
gpca
ggsave(gpca, filename = "Scree_IMP_GD - Final.png",  bg = "transparent")
