...
dados <- subset(GRADUADOS, select = -c(REGISTRO))
...
qgraph(cor(matcor), title="Correlações Grau de Exigência - Graduados",
       # layout = "spring", 
...
qgraph(cor(matcor), layout="spring", shape="rectangle", 
       ...
       title = "Correlações Grau de Exigência - Graduados")
...
gpca <- fviz_eig(pca, addlabels=FALSE, ylim = c(0,60), barfill = "deeppink4")
...
gpca <- gpca +
  ...
  ggtitle("Scree Plot - Exigência - Graduados") +
  ylab("Porcentagem de Variância Explicada") +
  xlab("Dimensões")
gpca
ggsave(gpca, filename = "Scree_Exi_GD.png",  bg = "transparent")
...
#O fator 1 responde por 54,89% da variância total. Da mesma forma, o segundo fator 
#responde por 7,03% da variância total. Várias considerações devem integrar a análise do número
...
PCAcompetencias <-principal(dados_matrix, nfactors=2, n.obs = 851,
...
PCACompetenciasVarimax<-principal(dados_matrix, nfactors=2,
                                  n.obs=851,rotate="varimax",scores=TRUE)
PCACompetenciasVarimax

print(loadings(PCACompetenciasVarimax), cutoff=0.7)
